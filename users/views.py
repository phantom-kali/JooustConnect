...
<<<<<<< HEAD
    if request.method == "POST":
        ...
        transaction_id, amount, transaction_date, phone_number = extract_mpesa_details(
            mpesa_message, action="verify"
        )
                ...
                try:
                    ...
                    if not transaction.is_used:
                        if (
                            ...
                            and transaction.phone_number == phone_number
                        ):
=======
    if request.method == 'POST':
        mpesa_message = request.POST.get('mpesa_message')
...
<<<<<<< HEAD
                            ...
                            messages.success(
                                request, "Premium subscription activated successfully!"
                            )
                            return redirect("premium_dashboard")
                        else:
                            messages.error(
                                request, "Transaction details do not match our records."
                            )
                    else:
                        messages.error(
                            request, "This transaction has already been used."
                        )
                ...
                messages.error(
                    request,
                    f"The amount paid (Ksh {amount}) does not match the required fee (Ksh {premium_fee}).",
                )
        else:
            messages.error(
                request, "Invalid M-Pesa message. Please check and try again."
            )
...
<<<<<<< HEAD
    if filter_by == "likes":
        user_posts = user_posts.annotate(like_count=Count("likes")).order_by(
            "-like_count"
        )
    elif filter_by == "comments":
        user_posts = user_posts.annotate(comment_count=Count("comments")).order_by(
            "-comment_count"
        )
...
<<<<<<< HEAD
    return redirect(
        request.META.get("HTTP_REFERER", "profile", username=user_to_follow.username)
    )
...
<<<<<<< HEAD
    return redirect(
        request.META.get("HTTP_REFERER", "profile", username=user_to_unfollow.username)
    )
=======
...
<<<<<<< HEAD
    if request.method == "POST":
        ...
        if form.is_valid():
            user = form.save()
            request.session["theme"] = user.theme_preference
            messages.success(
                request, "Settings saved successfully. The theme has been updated."
            )
...
=======
    if request.method == 'POST':
        ...
        if form.is_valid():
            user = form.save()
            request.session['theme'] = user.theme_preference
            messages.success(request, 'Settings saved successfully. The theme has been updated.')
            return redirect('settings')
    else:
        form = UserSettingsForm(instance=request.user)
